    ...
    return (
      <div className="min-h-screen flex flex-col justify-center items-center bg-[#fff]">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push(`/event-id/${wwwId}/invitation`)}
    ...
    <>
      ...
      <div className="flex flex-col items-center justify-center flex-1 py-8 md:py-12 px-4">
        <div className="relative w-full max-w-[900px] bg-white rounded-2xl border border-[#B7B7B7] p-0 shadow-md mx-auto z-10" style={{ minHeight: 700 }}>
          ...
          {/* Main Content */}
          <div className="w-full max-w-[1200px] mx-auto flex flex-col items-center mb-8 mt-2 z-10 px-4 md:px-16 pt-8 md:pt-12 pb-8">
            <div className="text-center w-full mb-8 md:mb-10">
              <div className="flex flex-col items-center">
                <span className="text-2xl md:text-3xl lg:text-4xl" style={{ fontFamily: 'Great Vibes, cursive', fontWeight: 500, color: '#E5B574', letterSpacing: '0.5px', lineHeight: 1.1 }}>{t.invitation.whatsYour}</span>
                <span className="text-3xl md:text-4xl lg:text-5xl" style={{ fontFamily: 'Great Vibes, cursive', fontWeight: 500, color: '#08080A', letterSpacing: '0.5px', lineHeight: 1.1, marginTop: '0.5rem' }}>{t.invitation.mealPreference}</span>
                <div className="w-24 h-[2px] bg-[#B7B7B7] mx-auto my-4" />
              </div>
            </div>
            {/* Guest Meal Selection */}
            <div className="w-full max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6 md:gap-y-8 mb-8 md:mb-12">
              {guests.map((guestName, idx) => (
                <div key={guestName} className={`flex flex-col w-full items-center md:items-${idx % 2 === 0 ? 'start' : 'end'}`}>
                  <div className={`text-sm md:text-base text-[#08080A] uppercase mb-2 text-center md:${idx % 2 === 0 ? 'text-left' : 'text-right'}`} style={{ fontFamily: 'Montserrat', letterSpacing: '0.5px' }}>{guestName}</div>
                  <div className="flex flex-col gap-3 md:gap-4 w-full">
                    {options.map(option => (
                      <button
                        key={option.value}
                        className={`w-full py-3 rounded-md text-sm md:text-base transition-colors focus:outline-none text-center md:${idx % 2 === 0 ? 'text-left pl-6' : 'text-right pr-6'} ${selections[guestName] === option.value ? 'bg-[#08080A] text-white' : 'bg-[#F5F5F5] text-[#08080A]'}`}
