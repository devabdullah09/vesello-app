...
export default function DynamicMainGalleryPage() {
  ...
  const handleCopyLink = () => {
    ...
    const partyLink = window.location.origin + `/${wwwId}`;
  ...
  // Load gallery counts and event data on component mount
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <EventHeader 
        ...
        currentPage="gallery"
      />
      <div className="flex-1 bg-white py-10 px-2 md:px-0 relative overflow-x-hidden pt-20" style={{ fontFamily: 'Montserrat, Arial, Helvetica, sans-serif' }}>
        <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)]">
          <div className="relative w-full max-w-5xl bg-white rounded-2xl border border-[#C7B299] p-8 md:p-16 shadow-md mx-auto z-10" style={{ minHeight: 700 }}>
            ...
            <div className="absolute left-8 top-8 text-xl md:text-2xl text-[#08080A]" style={{ fontFamily: 'Montserrat', fontWeight: 400 }}>
              {t.gallery.welcomeTo} {t.gallery.title}
            </div>
            ...
            <div className="flex flex-col justify-center relative z-10">
              <div className="text-center mt-2 mb-8">
                <div className="text-base md:text-lg" style={{ fontFamily: 'Montserrat', fontWeight: 400, color: '#08080A' }}>{t.gallery.welcomeTo}</div>
                <div className="text-4xl md:text-5xl font-sail" style={{ fontWeight: 400, marginTop: 4, marginBottom: 0, letterSpacing: '0.5px', lineHeight: 1.1, color: '#08080A' }}>
                  {eventData?.coupleNames || t.status.loading}
                </div>
                ...
                <div className="text-sm mt-2" style={{ color: '#888', fontWeight: 400, fontFamily: 'Montserrat', fontSize: '16px', letterSpacing: '0.01em', lineHeight: 1.4 }}>
                  {eventData?.eventDate ? new Date(eventData.eventDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : t.status.loading}
                </div>
              </div>
                ...
                <div className="flex flex-col md:flex-row gap-8 justify-center items-center w-full mb-8 flex-wrap">
                  {customAlbums.map((album) => (
                    <div key={album.id} className="flex flex-col items-center">
                      <Link href={`/${wwwId}/gallery/album/custom-${album.id}`} className="relative w-[270px] h-[220px] md:w-[370px] md:h-[260px] mb-2 group block">
                        <Image 
              ...
              <div className="flex flex-col items-center mt-4 mb-2">
                <div className="text-lg font-medium mb-3 text-[#08080A] text-center" style={{ fontFamily: 'Montserrat', fontWeight: 600, fontSize: '16px', color: '#08080A', letterSpacing: '0.01em', lineHeight: 1.4 }}>{t.gallery.knowSomeone}</div>
                ...
                onClick={handleCopyLink}>
                  {t.gallery.copyPartyLink}</button>
              </div>
            </div>
          </div>
