...
export default function EventPartyDayGallery() {
  ...
  const loadGalleryFiles = async () => {
    try {
      const response = await fetch(`/api/${wwwId}/gallery?albumType=party-day&mediaType=${tab}`);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      {showSuccess && (
        <UploadSuccessOverlay
          onViewGallery={() => router.push(`/${wwwId}/gallery/main`)}
          onCountMeIn={() => {
            setShowSuccess(false);
            router.push(`/${wwwId}#team-section`);
          }}
        />
      )}
