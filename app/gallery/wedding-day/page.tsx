...
export default function WeddingDayGallery() {
  ...
  const loadGalleryFiles = async () => {
    try {
      console.log('Loading gallery files for:', 'wedding-day', tab);
      const response = await fetchGalleryFiles('wedding-day', tab);
      console.log('Gallery files response:', response);
      if (response.success) {
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-white py-10 px-4 md:px-0 relative overflow-x-hidden" style={{ fontFamily: 'Montserrat, Arial, Helvetica, sans-serif' }}>
        <div className="relative w-full max-w-5xl bg-white rounded-2xl border border-[#C7B299] p-4 md:p-8 lg:p-12 shadow-md mx-auto z-10" style={{ minHeight: 500 }}>
          {/* Decorative Corners and Sparkles */}
         
          <CdnImage src="/images/Gallery/bottom-left-sparkle.png" alt="bottom left sparkle" width={202} height={32} className="absolute left-3 bottom-4 z-0 hidden md:block" />
          <CdnImage src="/images/Gallery/middle-right-sparkle.png" alt="middle right sparkle" width={280} height={42} className="absolute right-5 top-1/4 z-0 hidden md:block" />
          <CdnImage src="/images/Gallery/over-leaf-sparkle.png" alt="over leaf sparkle" width={252} height={32} className="absolute left-5 top-20 z-0 hidden md:block" />

          {/* Main Content */}
          <div className="flex flex-col relative z-10">
            <div className="flex flex-col md:flex-row justify-between items-start mb-4 gap-4">
              <div className="text-xl md:text-2xl lg:text-3xl font-semibold text-[#08080A]" style={{ fontFamily: 'Montserrat', fontWeight: 500 }}>
                {t.gallery.weddingDay}
              </div>
              <div className="flex flex-col items-start md:items-end">
                <div className="text-sm md:text-base lg:text-lg font-semibold text-[#08080A] flex items-center" style={{ fontFamily: 'Montserrat', fontWeight: 500 }}>
                  {t.gallery.uploadPhotosVideos} {downloadIcon}
                </div>
                <div className="text-xs text-[#888] mt-1" style={{ fontFamily: 'Montserrat', fontWeight: 400 }}>
                  {t.gallery.uploadedBy} 20.06.2025
                </div>
              </div>
            </div>
            {/* Tabs */}
            <div className="flex flex-row border-b border-[#C7B299] mb-4">
              <button onClick={() => setTab('photos')} className={`font-semibold mr-6 pb-2 border-b-2 text-sm md:text-base ${tab === 'photos' ? 'text-[#C18037] border-[#C18037]' : 'text-[#08080A] border-transparent'}`} style={{ fontFamily: 'Montserrat', fontWeight: 500 }}>
                {t.gallery.photos}
              </button>
              <button onClick={() => setTab('videos')} className={`font-semibold pb-2 border-b-2 text-sm md:text-base ${tab === 'videos' ? 'text-[#C18037] border-[#C18037]' : 'text-[#08080A] border-transparent'}`} style={{ fontFamily: 'Montserrat', fontWeight: 500 }}>
                {t.gallery.videos}
              </button>
            </div>
            {/* Image/Video Grid */}
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4 lg:gap-6">
              {/* Upload Card */}
              <div className={`relative w-full aspect-square rounded-lg overflow-hidden group border ${tab === 'photos' ? 'border-[#E5B574]' : 'border-[#C18037]'}`}>
                <CdnImage src="/images/Gallery/maingallery.jpg" alt="Upload" fill style={{ objectFit: 'cover' }} />
                <div className={`absolute inset-0 ${tab === 'photos' ? 'bg-[#E5B574]/70' : 'bg-[#C18037]/70'} flex flex-col items-center justify-center opacity-100 group-hover:opacity-100 transition-opacity duration-300`}>
                  <button onClick={() => fileInputRef.current?.click()} className="border border-white text-white rounded px-4 md:px-6 py-1 bg-transparent hover:bg-white hover:text-[#C18037] transition font-semibold text-xs md:text-sm lg:text-base" style={{ fontFamily: 'Montserrat', fontWeight: 600, letterSpacing: '0.01em', lineHeight: 1.4 }}>{t.gallery.upload}</button>
                  <input ref={fileInputRef} type="file" accept={tab === 'photos' ? 'image/*' : 'video/*'} multiple capture="environment" className="hidden" onChange={handleUpload} />
                </div>
              </div>
              {/* Gallery Images or Videos */}
              {tab === 'photos' && images.map((img, i) => (
                <div key={i} className="relative w-full aspect-square rounded-lg overflow-hidden cursor-pointer group" onClick={() => openPreview('image', i)}>
                  <CdnImage 
                    src={img} 
                    alt={`Gallery ${i}`} 
                    fill 
              ...
              {tab === 'videos' && videos.map((vid, i) => (
                <div key={i} className="relative w-full aspect-square rounded-lg overflow-hidden cursor-pointer group" onClick={() => openPreview('video', i)}>
                  <video className="w-full h-full object-cover" poster={vid.thumb}>
